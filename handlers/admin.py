...
async def show_history(message: Message):
    ...
    if stats['avg_final_price'] > 0:
        text += f"Средняя цена продажи: {format_price(stats['avg_final_price'])} тенге\n"
...
async def show_history_lots(callback: CallbackQuery):
    ...
    for lot in lots:
        ...
        text += f"🏙️ {lot['city']}\n"
        text += f"💰 Старт: {format_price(lot['start_price'])} тенге\n"

        if lot.get('current_price') and lot['current_price'] > lot['start_price']:
            text += f"🔥 Финал: {format_price(lot['current_price'])} тенге\n"
